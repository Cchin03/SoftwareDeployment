      ...
      <div className="mx-auto max-w-7xl px-4 pb-16 sm:px-6">
        ...
        <div className="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div className="relative w-full sm:w-72">
            <svg className="absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 text-zinc-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {filteredProducts.map((product, index) => {
              ...
              return (
                <Link
                  ...
                  className="group flex flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white transition-all duration-200 hover:-translate-y-1 hover:shadow-lg"
                >
                  <div className="relative flex h-52 items-center justify-center overflow-hidden bg-zinc-50 p-6 text-6xl">
                    ...
                    {product.tag && (
                      <span className={`absolute top-3 left-3 rounded-full px-2 py-0.5 text-[10px] font-bold ${tagColors[product.tag] ?? "bg-zinc-100 text-zinc-600"}`}>
                        {product.tag}
                      </span>
                    )}
                  </div>
                  <div className="flex flex-1 flex-col p-4">
                    <p className="mb-1 text-xs font-medium uppercase tracking-[0.2em] text-zinc-400">{product.brand}</p>
                      ...
                      <div className="mb-2 flex items-center gap-1">
                        <div className="flex">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <svg key={i} className={`h-3 w-3 ${i < rating ? "text-amber-400" : "text-zinc-200"}`} fill="currentColor" viewBox="0 0 20 20">
