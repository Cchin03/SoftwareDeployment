...
export default function ShopDashboard() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (!user) {
        // No session at all → sign in as guest automatically
  ...
  /* ── Guard: if guest tries to access cart ── */
  const handleCartAction = (e: React.MouseEvent) => {
    if (isGuest) {
      e.preventDefault()
      setShowModal(true) // show register prompt
    }
    // if logged in, navigate normally to /cart
  }
  ...
  const handleLogout = async () => {
    ...
    await supabase.auth.signInAnonymously() // ← become a guest after logout
    setIsGuest(true)  // ← manually set guest state
      ...
      <section className="bg-white border-y border-zinc-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {deals.map((d) => (
              <div key={d.name} className="bg-zinc-50 rounded-2xl border border-zinc-200 p-5 hover:shadow-md transition-all duration-200 hover:-translate-y-0.5">
                ...
                {/* Add to cart — blocked for guests */}
                <button
                  onClick={(e) => handleAddToCart(e, d.name)}
                  className={`mt-3 w-full text-sm font-semibold py-2 rounded-xl transition-colors flex items-center justify-center gap-1 ${
                    isGuest
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm p-8 shadow-2xl text-center">
            ...
            <h2 className="text-xl font-bold text-zinc-900 mb-2">Account Required</h2>
            <p className="text-sm text-zinc-500 mb-6">
              You need to create an account or sign in to add items to your cart and place orders.
            </p>
            <div className="flex flex-col gap-3">
              <Link
                href="/register"
                className="w-full h-11 bg-zinc-900 text-white text-sm font-semibold rounded-xl flex items-center justify-center hover:bg-zinc-700 transition"
              >
                Create account
              </Link>
              <Link
                href="/login"
                className="w-full h-11 border-2 border-zinc-200 text-zinc-700 text-sm font-semibold rounded-xl flex items-center justify-center hover:bg-zinc-50 transition"
              >
                Sign in
              </Link>
              <button
                onClick={() => setShowModal(false)}
                className="text-sm text-zinc-400 hover:text-zinc-600 transition mt-1"
              >
